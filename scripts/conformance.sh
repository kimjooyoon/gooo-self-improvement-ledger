...
jq -e '
  ...
  .summary == {total:12,closed:6,unknown:6,refuted:0} and
  ...
  (.cells|map(select(.numerator == 1 and .denominator == 1))|length) == 6 and
  (.cells|map(select(.state == "UNKNOWN"))|length) == 6 and
  ...
  .releases == {total:6,verified:6,unknown:0,refuted:0} and
