...
jq -S -n \
  --slurpfile comparison "$candidate_output/exact-pair-comparison.json" \
  --slurpfile report "$candidate_output/conformance-report.json" \
  '{schema:"gooo/content-addressed-evidence-projector/exact-pair-comparison/v1",case_count:($comparison[0].cases|length),semantic_root_equal:($comparison[0].cases|all(.semantic_root_equal)),canonical_evidence_equal:($comparison[0].cases|all(.canonical_evidence_equal)),inclusion_proofs_verified:($report[0].cases|all(all(.inclusion_proofs[];.verified==true))),replay_equal:$report[0].replay.match,scenario_identity_equal:($comparison[0].cases|all(all(.per_indicator_pairs[];.scenario != ""))),fixture_digest_equal:($comparison[0].cases|all(all(.per_indicator_pairs[];.fixture != ""))),contract_digest_equal:($comparison[0].cases|all(all(.per_indicator_pairs[];.contract != ""))),toolchain_equal:($comparison[0].cases|all(all(.per_indicator_pairs[];.toolchain=="go1.27.x"))),runner_equal:($comparison[0].cases|all(all(.per_indicator_pairs[];.runner=="github-actions/ubuntu-latest"))),job_equal:($comparison[0].cases|all(all(.per_indicator_pairs[];.job==$comparison[0].cases[0].per_indicator_pairs[0].job)))}' \
