...
mkdir -p "$caller_root/output"
"$output_binary" conformance --caller-root "$caller_root" --repository-root "$repository" --output "$caller_root/output/conformance.json"
cp "$caller_root/output/conformance.json" "$output_dir/conformance.json"
jq -e '
