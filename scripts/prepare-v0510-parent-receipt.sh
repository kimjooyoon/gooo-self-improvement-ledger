...
current_lock_set_digest="sha256:$(jq -cS 'del(.releases.measurement_boundary_v2_projector_durable_release,.releases.operational_provenance_projector_durable_release,.releases.self_improvement_frontier_projector_durable_release) | .releases' "$contract" | sha256sum | awk '{print $1}')"
...
if [ "$metadata_state" = REFUTED ] || [ "$tag_state" = REFUTED ] || [ "$target_state" = REFUTED ] || [ "$contents_state" = REFUTED ] || [ "$public_tag_state" = REFUTED ]; then
  ...
  unknown_class="PARENT_RECEIPT_CONTRADICTION"
elif [ "$metadata_state" = CLOSED ] && [ "$tag_state" = CLOSED ] && [ "$target_state" = CLOSED ] && [ "$contents_state" = CLOSED ] && [ "$public_tag_state" = CLOSED ] && [ "$current_count" -eq 62 ] && [ "$current_lock_set_digest" = "$parent_lock_set_digest" ]; then
