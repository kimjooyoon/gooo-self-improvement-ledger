...
jq -n --arg compiler "$(jq -r '.assets[0].sha256' <<< "$self_lock")" --arg reducer "$(jq -r '.assets[0].sha256' <<< "$reducer_lock")" --arg equivalence "$(jq -r '.assets[0].sha256' <<< "$equivalence_lock")" --arg wave "$(jq -r '.assets[0].sha256' <<< "$wave_lock")" '
  {fixture_id:"v0520-atomic-wave",expected_state:"CLOSED",base_ledger_digest:"ledger:wave-v1",evidence_digests:[$compiler,$reducer,$equivalence,$wave],proposals:[
  ...
  ]}' > "$wave_cases/v0520-atomic-wave.json"
...
jq -e '.decision=="CONFORMANT" and .scenario_denominator==1 and .closed==1 and .unknown==0 and .refuted==0 and .replay_match==true' "$wave_product/actual-cli-output.json" >/dev/null
jq -e '.schema=="gooo/semantic-wave-merge-projector/wave-projection/v1" and .scenario_denominator==1 and .state_counts=={total:1,closed:1,unknown:0,refuted:0} and .cases[0].state=="CLOSED" and .cases[0].accepted_wave==["proposal-cell-68","proposal-cell-69","proposal-cell-70","proposal-cell-71"] and .cases[0].deferred_frontier==[] and .cases[0].conflict_witnesses==[]' "$wave_product/actual/wave-projection.json" >/dev/null
