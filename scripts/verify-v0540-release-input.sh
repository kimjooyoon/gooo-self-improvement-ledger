...
if [ "$mode" = --repository ]; then
  ...
  jq -e '.schema=="gooo/self-improvement-ledger/atomic-v0540-adoption-wave/v1" and
    ...
    .preservation=={external_utility_state:"UNKNOWN",general_program_equivalence_claim:false,historical_refutations_preserved:true,improvement_aggregation:"NOT_CLAIMED",mutation_policy:"NO_DELETE_NO_OVERWRITE",v0_53_semantic_summary:"preserved_as_parent_release_evidence"} and
  ...
  test "$old_lock_digest" = "sha256:ffc92192ef7ba838e8f4917c5f7c97d878786a00a79f6079e0761f64a05001b3"
...
jq -e '.schema=="gooo/self-improvement-ledger/v0540-parent-lock-receipt/v1" and .parent.repository=="kimjooyoon/gooo-self-improvement-ledger" and .parent.tag=="v0.53.0" and .parent.release_id==380943341 and .parent.tag_object_sha=="dd204df84abecdd634e9321cc40b2714f91d96eb" and .parent.target_commit_sha=="e84c9209316cfa6d07d2ea96d988d05c8c6f7367" and .parent.parent_lock_set_digest=="sha256:ffc92192ef7ba838e8f4917c5f7c97d878786a00a79f6079e0761f64a05001b3" and .parent.release_lock_manifest=={contents_blob_sha:"73d61aba0158d3a4d3535f09c89ffb3f218b08cd",sha256:"sha256:17893af29dfafd11c1927c152372778127cc7d77199c045f7c3e808405b6f981",size_bytes:319611} and .primary.state=="CLOSED" and .primary.api_observation.selected==0 and .primary.api_observation.executed==0 and .primary.api_observation.reused==70 and .lock_set.current_count==72 and .lock_set.parent_count==70 and .lock_set.unchanged_70_lock_set==true and .full_fallback=={executed:0,required:false,reused:70,selected:0,state:"NOT_REQUIRED"} and .authority.repository_writes==0 and .authority.local_product_validation_executions==0 and .authority.cross_project_required_gates==0' "$parent_receipt" >/dev/null
jq -e '.schema=="gooo/self-improvement-ledger/v0540-live-lock-receipt/v1" and .parent_reuse.mode=="PARENT_V0530_RELEASE_RECEIPT_REUSE" and .parent_reuse.executed==0 and .parent_reuse.reused==70 and .parent_reuse.selected==0 and .changed_live.selected==2 and .changed_live.executed==2 and .changed_live.reused==0 and .changed_live.live_verified==2 and .changed_live.unknown==0 and .changed_live.refuted==0 and .full_72_lock_audit=={executed:false,required:false} and .authority=={cross_project_required_gates:0,local_product_validation_executions:0,repository_writes:0,verification:"GITHUB_ACTIONS"}' "$live_receipt" >/dev/null
