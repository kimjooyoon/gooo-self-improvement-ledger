...
jq -S -n --arg subject "${SUBJECT_SHA:-unknown}" --argjson report "$(cat "$artifact/report.json")" --argjson verification "$(cat "$artifact/releases/verification.json")" --argjson wave "$(cat "$artifact/atomic-v0560-wave-v1.json")" --argjson products "$(cat "$artifact/v0560-products/product-integration.json")" --argjson guard "$(cat "$artifact/v0560-products/release-lineage-guard-plan-assertions.json")" \
  '{schema:"gooo/self-improvement-portfolio/conformance/v1",subject_sha:$subject,decision:"CLOSED",summary:$report.summary,checks:{profile_state:true,proof_indicator_totals:true,one_to_one_bindings:true,parent_receipt_reuse:($verification.release_lock_snapshot.parent_reuse.reused==72),changed_lock_wave:($verification.release_lock_snapshot.changed_live.selected==5 and $verification.release_lock_snapshot.changed_live.executed==5),semantic_wave:($wave.semantic_wave.accepted_wave_order|length)==7,product_integration:(($products.products|length)==7),lineage_guard:($guard.policy_conformance.pass==true)},repository_writes:0,authority:{verification:"GITHUB_ACTIONS_ONLY",local_validation_commands:0,cross_project_required_gates:0},preservation:{external_utility:"UNKNOWN",whole_language_improvement:"UNKNOWN",improvement_aggregation:"NOT_CLAIMED",exact_before_after_pair:"UNKNOWN"}}' > "$artifact/conformance.json"
