#!/usr/bin/env bash
set -Eeuo pipefail
trap 'status=$?; if test -s "${artifact:-}/conformance.json"; then jq -S "{schema:.schema,decision:.decision,summary:.summary,checks:.checks,repository_writes:.repository_writes,authority:.authority,percentage:.percentage,score:.score}" "$artifact/conformance.json" >&2 || true; fi; echo "v0.56 conformance failed at line ${LINENO}: ${BASH_COMMAND}" >&2; exit "$status"' ERR
...
jq -e '.schema=="gooo/self-improvement-portfolio/conformance/v1" and .decision=="CLOSED" and .summary=={closed:83,refuted:4,total:88,unknown:1} and .checks.profile_state==true and .checks.proof_indicator_totals==true and .checks.one_to_one_bindings==true and .checks.parent_receipt_reuse==true and .checks.changed_lock_wave==true and .checks.semantic_wave==true and .checks.product_integration==true and .checks.lineage_guard==true and .repository_writes==0 and .authority=={cross_project_required_gates:0,local_validation_commands:0,verification:"GITHUB_ACTIONS_ONLY"} and (.percentage==null) and (.score==null)' "$artifact/conformance.json" >/dev/null
jq -S '{schema_ok:(.schema=="gooo/self-improvement-portfolio/conformance/v1"),decision_ok:(.decision=="CLOSED"),summary_ok:(.summary=={closed:83,refuted:4,total:88,unknown:1}),profile_state_ok:(.checks.profile_state==true),proof_indicator_totals_ok:(.checks.proof_indicator_totals==true),one_to_one_bindings_ok:(.checks.one_to_one_bindings==true),parent_receipt_reuse_ok:(.checks.parent_receipt_reuse==true),changed_lock_wave_ok:(.checks.changed_lock_wave==true),semantic_wave_ok:(.checks.semantic_wave==true),product_integration_ok:(.checks.product_integration==true),lineage_guard_ok:(.checks.lineage_guard==true),repository_writes_ok:(.repository_writes==0),authority_ok:(.authority=={cross_project_required_gates:0,local_validation_commands:0,verification:"GITHUB_ACTIONS_ONLY"}),percentage_absent:(.percentage==null),score_absent:(.score==null)}' "$artifact/conformance.json" >&2
