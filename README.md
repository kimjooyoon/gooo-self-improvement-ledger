...
language. The denominator is exactly 47 named cells, each bound one-to-one to
...
`STAGED_QUASIQUOTE_DURABLE_RELEASE`.

The denominator migration is explicit and append-only: `46 -> 47` with
`ADD1/RETIRE0/SPLIT0`. The proof buckets are `FOUNDATION/COHERENCE/REGRESSION`
`4/38/5`, and the indicator buckets are `DRIVER/OUTCOME/GUARDRAIL` `4/38/5`.
...
twenty-six independent evidence inputs without treating its entry count as the portfolio
...
commit, and consumer asset identities for the forty-four immutable inputs
