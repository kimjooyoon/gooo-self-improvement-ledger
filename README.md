...
language. The denominator is exactly 48 named cells, each bound one-to-one to
...
`ERROR_DIRECTED_EVOLUTION_PLANNER_DURABLE_RELEASE`.

The denominator migration is explicit and append-only: `47 -> 48` with
`ADD1/RETIRE0/SPLIT0`. The proof buckets are `FOUNDATION/COHERENCE/REGRESSION`
`4/39/5`, and the indicator buckets are `DRIVER/OUTCOME/GUARDRAIL` `4/39/5`.
...
twenty-seven independent evidence inputs without treating its entry count as the portfolio
denominator. Current registry disposition is `23 CLOSED / 0 UNKNOWN / 4
...
commit, and consumer asset identities for the forty-five immutable inputs
