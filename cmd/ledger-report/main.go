...
func validateProfile(profile Profile) {
	...
	if profile.TotalCells != 37 || len(profile.Cells) != profile.TotalCells {
		fatalf("profile must contain exactly 37 cells")
	}
	...
	if !equalIntMap(profile.ProofTotals, map[string]int{"FOUNDATION": 4, "COHERENCE": 28, "REGRESSION": 5}) {
		fatalf("proof totals must be FOUNDATION4/COHERENCE28/REGRESSION5")
	}
	if !equalIntMap(profile.IndicatorTotals, map[string]int{"DRIVER": 4, "OUTCOME": 28, "GUARDRAIL": 5}) {
		fatalf("indicator totals must be DRIVER4/OUTCOME28/GUARDRAIL5")
	}
