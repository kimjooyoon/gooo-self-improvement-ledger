...
func validateProfile(profile Profile) {
	...
	if profile.TotalCells != 36 || len(profile.Cells) != profile.TotalCells {
		fatalf("profile must contain exactly 36 cells")
	}
	...
	if !equalIntMap(profile.ProofTotals, map[string]int{"FOUNDATION": 4, "COHERENCE": 27, "REGRESSION": 5}) {
		fatalf("proof totals must be FOUNDATION4/COHERENCE27/REGRESSION5")
	}
	if !equalIntMap(profile.IndicatorTotals, map[string]int{"DRIVER": 4, "OUTCOME": 27, "GUARDRAIL": 5}) {
		fatalf("indicator totals must be DRIVER4/OUTCOME27/GUARDRAIL5")
	}
