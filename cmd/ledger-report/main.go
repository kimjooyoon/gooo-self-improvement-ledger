...
func validateProfile(profile Profile) {
	...
	if profile.TotalCells != 35 || len(profile.Cells) != profile.TotalCells {
		fatalf("profile must contain exactly 35 cells")
	}
	...
	if !equalIntMap(profile.ProofTotals, map[string]int{"FOUNDATION": 4, "COHERENCE": 26, "REGRESSION": 5}) {
		fatalf("proof totals must be FOUNDATION4/COHERENCE26/REGRESSION5")
	}
	if !equalIntMap(profile.IndicatorTotals, map[string]int{"DRIVER": 4, "OUTCOME": 26, "GUARDRAIL": 5}) {
		fatalf("indicator totals must be DRIVER4/OUTCOME26/GUARDRAIL5")
	}
	...
	if !equalIntMap(actualProof, profile.ProofTotals) || !equalIntMap(actualIndicator, profile.IndicatorTotals) {
		fatalf("profile cell classification totals do not match declared 4/26/5")
	}
}
