...
func validateProfile(profile Profile) {
	...
	if profile.TotalCells != 27 || len(profile.Cells) != profile.TotalCells {
		fatalf("profile must contain exactly 27 cells")
	}
	...
	if !equalIntMap(profile.ProofTotals, map[string]int{"FOUNDATION": 4, "COHERENCE": 18, "REGRESSION": 5}) {
		fatalf("proof totals must be FOUNDATION4/COHERENCE18/REGRESSION5")
	}
	if !equalIntMap(profile.IndicatorTotals, map[string]int{"DRIVER": 4, "OUTCOME": 18, "GUARDRAIL": 5}) {
		fatalf("indicator totals must be DRIVER4/OUTCOME18/GUARDRAIL5")
	}
	...
	if !equalIntMap(actualProof, profile.ProofTotals) || !equalIntMap(actualIndicator, profile.IndicatorTotals) {
		fatalf("profile cell classification totals do not match declared 4/18/5")
	}
}
