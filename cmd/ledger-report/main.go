...
func validateProfile(profile Profile) {
	...
	if profile.TotalCells != 26 || len(profile.Cells) != profile.TotalCells {
		fatalf("profile must contain exactly 26 cells")
	}
	...
	if !equalIntMap(profile.ProofTotals, map[string]int{"FOUNDATION": 4, "COHERENCE": 17, "REGRESSION": 5}) {
		fatalf("proof totals must be FOUNDATION4/COHERENCE17/REGRESSION5")
	}
	if !equalIntMap(profile.IndicatorTotals, map[string]int{"DRIVER": 4, "OUTCOME": 17, "GUARDRAIL": 5}) {
		fatalf("indicator totals must be DRIVER4/OUTCOME17/GUARDRAIL5")
	}
	...
	if !equalIntMap(actualProof, profile.ProofTotals) || !equalIntMap(actualIndicator, profile.IndicatorTotals) {
		fatalf("profile cell classification totals do not match declared 4/17/5")
	}
}
