...
func validateProfile(profile Profile) {
	...
	if profile.TotalCells != 47 || len(profile.Cells) != profile.TotalCells {
		fatalf("profile must contain exactly 47 cells")
	}
	...
	if !equalIntMap(profile.ProofTotals, map[string]int{"FOUNDATION": 4, "COHERENCE": 38, "REGRESSION": 5}) {
		fatalf("proof totals must be FOUNDATION4/COHERENCE38/REGRESSION5")
	}
	if !equalIntMap(profile.IndicatorTotals, map[string]int{"DRIVER": 4, "OUTCOME": 38, "GUARDRAIL": 5}) {
		fatalf("indicator totals must be DRIVER4/OUTCOME38/GUARDRAIL5")
	}
	...
	if !equalIntMap(actualProof, profile.ProofTotals) || !equalIntMap(actualIndicator, profile.IndicatorTotals) {
		fatalf("profile cell classification totals do not match declared 4/38/5")
	}
}
